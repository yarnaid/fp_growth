...
int main(int argc, char const *argv[]) {
    ...
    const double minimum_support_treshold = 0.03;  // in %, but also available in absolute value
    const double maximum_supprot_treshold = 0.99;  // in %, but also available in absolute value
...
//    print_pattern(patterns);
    ...
    mark_tree(fptree, true);  // mark campaigns and save them to files
    ...
    build_classifier(patterns, test_vector, 0.8, 11);

    // RESULT 2
    std::cout << "RESULT2" << std::endl;
    labeling(fptree, std::string(""));
